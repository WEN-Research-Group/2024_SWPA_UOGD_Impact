...
This project evaluates the impact of Unconventional Oil and Gas Development (UOGD) on groundwater quality within the Marcellus Shale region of southwestern Pennsylvania, addressing persistent concerns regarding potential contamination. While prior studies analyzing data from 2008 to 2018 suggested regional associations between drilling activity and elevated salinity, our 2024 investigation sought to determine if these patterns persist under current conditions.

We collected and analyzed 97 water samples from private wells, springs, and streams using a targeted sampling strategy focused on previously identified chloride hotspots, decommissioned wastewater impoundments, and recent spill sites, alongside appropriate control locations. Water quality was evaluated against health-based standards, and statistical comparisons were conducted between samples near the aforementioned sites and controls. These analyses accounted for complex geologic and topographic variables using fixed-effect modeling and watershed delineation.

Our findings indicate that widespread, regional groundwater degradation attributable to UOGD is not evident in the recent data. Observed impacts appear to be rare and highly localized rather than systemic. For example, among five spill sites investigated in detail, only one groundwater sample exhibited elevated salinity and brine-associated signatures. Geochemical and isotopic analyses suggest this anomaly resulted from localized influence via deeper groundwater flow paths rather than surface transport.

We also emphasized the importance of combining broad regional screening with targeted, hydrogeology-informed monitoring when assessing potential UOGD-related groundwater contamination.
...
All scripts used for statistical and geospatial analyses are included in the `Scripts` folder. Outputs generated by these scripts are saved in the `Results` folder. 
...
Figures and tables from the main text and SI are stored in the `Results/Figures` and `Results/Tables` folders, respectively.
...
Research described in this project was conducted under contract to the Health Effects Institute (HEI), an organization jointly funded by the United States Environmental Protection Agency (EPA) (Contract No. 68HERC19D0010) and certain oil and natural gas companies. Although the research was produced with partial funding by EPA and industry, they have not been subject to their review, and therefore the research does not necessarily reflect the views of the Agency or the oil and natural gas industry, and no official endorsement by the Agency or the industry should be inferred.
